...
| `HttpRiskScorer` | `RiskScorer` | port implementation, failures -> `Unavailable` | out -> port | `adapter/out/risk` | Observed |
...
`compose.yaml` adds `wiremock/wiremock:3.13.2`; CI needs Docker on `ubuntu-latest`.
