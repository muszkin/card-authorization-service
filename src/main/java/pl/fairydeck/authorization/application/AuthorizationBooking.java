 ...
 * the decision together with its hold and its outbox event. Everything that talks to the network happens
 * before this class is called, so the transaction never waits on a remote system.
 *
...
public class AuthorizationBooking {
    ...
    public AuthorizationBooking(LedgerRepository ledger, AuthorizationRepository authorizations, Outbox outbox,
    ...
    public Authorization book(Purchase purchase, Card card, RiskAssessment risk) {
        ...
        Authorization authorization = policy.authorize(purchase, card, balance, risk, now);
